...
public class OrderServiceImpl implements IOrderService {
    ...
    public ServerResponse createOrder(Integer userId, Integer shippingId) {
        ...
        //step1:参数非空校验
        if(shippingId==null){
            return  ServerResponse.serverResponseByError(ResponseCode.ERROR,"收货地址必填");
        }
        if(!shippingService.findShippingById(shippingId).isSuccess()){
    ...
    public ServerResponse selectByOrderNo(Integer userId,Long orderNo) {
        ...
        List<Order> orderList=orderMapper.findOrderItemByOrderNoAnduserId(userId,orderNo);
    ...
    public ServerResponse selectpay(Integer userId) {
        ...
        orderList=orderMapper.selectpay(userId);
        ...
        if(orderVOList==null||orderVOList.size()==0){
            return ServerResponse.serverResponseByError(ResponseCode.ERROR,"没有找到支付订单");
        }else {
    ...
    public ServerResponse pay() {
        List<Order> orderList=Lists.newArrayList();
        System.out.println(orderList.size());
        orderList=orderMapper.pay();
        List<OrderVO> orderVOList=Lists.newArrayList();
        List<OrderItemVO> orderItemVOList=new ArrayList<OrderItemVO>();
        for(Order order:orderList){
            List<OrderItem> orderItemList=orderItemMapper.findOrderItem(order.getOrderNo());
            OrderVO orderVO=assemble(order,orderItemList,order.getShippingId());
            orderVOList.add(orderVO);
        }
    ...
    public ServerResponse selectnopay(Integer userId) {
        List<Order> orderList=Lists.newArrayList();
          orderList=orderMapper.selectnopay(userId);
        List<OrderVO> orderVOList=Lists.newArrayList();
       List<OrderItemVO> orderItemVOList=new ArrayList<OrderItemVO>();
        for(Order order:orderList){
        ...
        if(orderVOList==null||orderVOList.size()==0){
            return ServerResponse.serverResponseByError(ResponseCode.ERROR,"没有找到未支付订单");
        }else {
            return ServerResponse.serverResponseBySuccess(orderVOList);
        }
    }
    ...
    public ServerResponse allorder(Integer userId) {
        List<Order> orderList=Lists.newArrayList();
        orderList=orderMapper.allorder(userId);
        List<OrderVO> orderVOList=Lists.newArrayList();
        List<OrderItemVO> orderItemVOList=new ArrayList<OrderItemVO>();
        for(Order order:orderList){
            List<OrderItem> orderItemList=orderItemMapper.findOrderItemByOrderNo(order.getOrderNo());
    ...
    public ServerResponse manageallorder(Integer pageNume,Integer pageSize) {
        ...
        List<Order> orderList=Lists.newArrayList();
        orderList=orderMapper.manageallorder();
        List<OrderVO> orderVOList=Lists.newArrayList();
        List<OrderItemVO> orderItemVOList=new ArrayList<OrderItemVO>();
        for(Order order:orderList){
            List<OrderItem> orderItemList=orderItemMapper.findOrderItemByOrderNo(order.getOrderNo());
    ...
    public ServerResponse send(Integer userId) {
        List<Order> orderList=Lists.newArrayList();
        orderList=orderMapper.send(userId);
        List<OrderVO> orderVOList=Lists.newArrayList();
        List<OrderItemVO> orderItemVOList=new ArrayList<OrderItemVO>();
        for(Order order:orderList){
            List<OrderItem> orderItemList=orderItemMapper.findOrderItemByOrderNo(order.getOrderNo());
    ...
    public ServerResponse cancel(Integer userId, Long orderNo) {
        ...
        //step4:返回结果
        order.setStatus(OrderStatusEnum.ORDER_CANCLE.getStatus());
    ...
    public ServerResponse deles(String orderNos) {
        ...
        String[] productIdsArr = orderNos.split(",");
        ...
        //step3:调用dao
        orderMapper.deles(productList);
        orderItemMapper.deles(productList);
        //step4:返回结果
        return ServerResponse.serverResponseBySuccess();
    }
    @Override
    ...
    public ServerResponse updateByuserIdAndOrderNo( Long orderNo) {
        ...
        int result=orderMapper.updateByuserIdAndOrderNo(orderNo);
        if(result==0){
            return ServerResponse.serverResponseByError(ResponseCode.ERROR,"没有找到订单");
        }else {
    ...
    public String callback(Map<String, String> requestParams) {
       ...
       if(trade_status.equals("TRADE_SUCCESS")){
           ...
           int result=orderMapper.updateBystatusandpayment(order1);
           if(result<=0){
              return "fail";
           }
       }
    ...
    public ServerResponse pay(Order order) {
        ...
        AlipayTradePrecreateRequestBuilder builder = new AlipayTradePrecreateRequestBuilder()
                ...
                .setNotifyUrl("182.92.97.77")//支付宝服务器主动通知商户服务器里指定的页面http路径,根据需要设置
        ...
        switch (result.getTradeStatus()) {
            case SUCCESS:
                ...
                // 需要修改为运行机器上的路径
                String filePath = String.format("f:/upload/qrcode/qr-%s.png",
                        response.getOutTradeNo());
                log.info("filePath:" + filePath);
                ZxingUtils.getQRCodeImge(response.getQrCode(), 256, filePath);
                PayVO payVO=new PayVO(order.getOrderNo(),qrHost+"qr-"+response.getOutTradeNo()+".png");
    ...
    private ServerResponse reduceProductStock(List<OrderItem> orderItemList){
        for(OrderItem orderItem:orderItemList){
         ...
         ServerResponse<Product> serverResponse=productService.findProduceByProductId(productId);
    ...
    private ServerResponse saveOrderItems(List<OrderItem> orderItemList,Order order){
        ...
        if(result!=orderItemList.size()){
            return ServerResponse.serverResponseByError(ResponseCode.ERROR,"订单明细插入失败");
        }
        return ServerResponse.serverResponseBySuccess();
    }
    //step4:创建Order实体类并保存到DB---->order
    ...
    private  ServerResponse getCartOrderItem(Integer userId,List<Cart> cartList){
        ...
        for(Cart cart:cartList){
            ...
            ServerResponse<Product> serverResponse=productService.findProduceByProductId(cart.getProductId());
            if(!serverResponse.isSuccess()){
                return serverResponse;
            }
            Product product=serverResponse.getData();
            ...
            orderItem.setProductImageHost(imageHost);
